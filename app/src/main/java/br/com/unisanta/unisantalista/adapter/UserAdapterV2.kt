...
class UserAdapterV2(private val tarefas: List<Tarefa>) :
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val cbIsFinished: CheckBox = itemView.findViewById(R.id.cb_is_finished)
    }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.cbIsFinished.setOnCheckedChangeListener(null)

        holder.cbIsFinished.isChecked = user.isFinished
        holder.cbIsFinished.text = if (user.isFinished) "Finalizado" else "Pendente"

        holder.cbIsFinished.setOnCheckedChangeListener { _, isChecked ->
            user.isFinished = isChecked
            holder.cbIsFinished.text = if (isChecked) "Finalizado" else "Pendente"
        }
    }
}